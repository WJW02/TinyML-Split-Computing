from flask_server.logger.Logger import Logger

logger = Logger(config_path='logger_config.json').get_logger(__name__)


class OffloadingAlgo:
    def __init__(self, avg_speed: float, num_layers: int, layers_sizes: list, inference_time_device: list,
                 inference_time_edge: list) -> None:
    ...
    @staticmethod
    def evaluation(initial_cost: int, layer_data_size: float, edge_computation_costs: list, avg_speed: float):
    ...
    def edge_only_computation_evaluation(self):
        ...
        self.lowest_evaluation = self.evaluation(
            ...
            edge_computation_costs=edge_computation_costs,
        )
